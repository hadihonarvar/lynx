...
| 41 | [`41_obligations.py`](41_obligations.py) | Obligations — mandatory side-actions on any verdict | "Attach `pre`/`post` obligations to a verdict (the XACML/Cedar model). A `pre` obligation gates the action — if its handler raises, the action is DENIED and the tool never runs (fail-closed); a `post` obligation fires after. The kernel ships no handlers — you pass an `ObligationRegistry` to `run_agent(..., obligations={...})`. Shows a credential gate denying a $50k refund while $500/$5k go through." |
